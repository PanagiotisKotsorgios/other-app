<!-- E:\call_center\app\Views\developer\projects\show.php -->
<?php
use App\Core\CSRF;
...
<div class="d-flex justify-content-between align-items-center mt-2 mb-3">
    <a href="<?= APP_URL ?>/developer/projects" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>All Projects</a>
</div>
...
<div class="row g-4">
    <div class="col-lg-8">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <table class="table table-borderless table-sm mb-0">
                            <tr><th class="text-muted fw-normal">Business</th><td class="fw-semibold"><?= htmlspecialchars($project['company_name']) ?></td></tr>
                            ...
                            <tr><th class="text-muted fw-normal">Budget</th><td class="fw-bold text-success">€<?= number_format($project['budget'],2) ?></td></tr>
                        </table>
                    </div>
                    <div class="col-md-6">
                        <table class="table table-borderless table-sm mb-0">
                            <tr><th class="text-muted fw-normal">Start Date</th><td><?= $project['start_date'] ? date('d M Y', strtotime($project['start_date'])) : '—' ?></td></tr>
                            <tr><th class="text-muted fw-normal">Deadline</th><td class="<?= $isOverdue?'text-danger fw-bold':'' ?>"><?= $project['deadline'] ? date('d M Y', strtotime($project['deadline'])) : '—' ?></td></tr>
                            <tr><th class="text-muted fw-normal">Tech Stack</th><td><?= htmlspecialchars($project['tech_stack'] ?? '—') ?></td></tr>
                        </table>
                    </div>
                </div>
        ...
        <!-- Update Status & URLs -->
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-semibold"><i class="bi bi-arrow-repeat me-1"></i> Update Status</div>
            <div class="card-body">
                <form method="POST" action="<?= APP_URL ?>/developer/projects/<?= $project['id'] ?>/status">
                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-4">
                            <label class="form-label small">Staging URL</label>
                        ...
                        <div class="col-md-4">
                            <button class="btn btn-sm btn-primary w-100"><i class="bi bi-save me-1"></i>Update</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <!-- Phases -->
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-semibold"><i class="bi bi-list-check me-1"></i> Phases</div>
            <div class="card-body p-0">
                <?php if(empty($phases)): ?>
                <div class="text-center text-muted py-4">No phases defined.</div>
                <?php else: ?>
                <table class="table table-hover mb-0">
                    <thead class="table-light"><tr><th>#</th><th>Phase</th><th>Status</th><th>Due Date</th><th>Completed</th></tr></thead>
                    <tbody>
    ...
    <div class="col-lg-4">
        ...
        <div class="card border-0 shadow-sm mb-3">
            ...
            <div class="card-body p-0">
                ...
                <div class="text-center text-muted py-3 small">No contracts.</div>
                <?php else: ?>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body p-0">
                ...
                <div class="text-center text-muted py-3 small">No invoices.</div>
                <?php else: ?>
                <ul class="list-group list-group-flush">
                    <?php foreach ($invoices as $inv): ?>
                    <li class="list-group-item">
                        <div class="d-flex justify-content-between">
                            <span class="small"><?= htmlspecialchars($inv['invoice_no'] ?? 'Invoice') ?></span>
                            <span class="badge <?= $inv['status']==='paid'?'bg-success':'bg-warning text-dark' ?>"><?= ucfirst($inv['status']) ?></span>
                        </div>
                        <div class="fw-bold">€<?= number_format($inv['total_amount'],2) ?></div>
                        <?php if($inv['filename']): ?>
                        <a href="<?= APP_URL ?>/admin/documents/invoices/<?= $inv['id'] ?>/download" class="btn btn-xs btn-outline-secondary" style="font-size:.7rem">Download</a>
                        <?php endif ?>
                    </li>
                    <?php endforeach ?>
                </ul>
