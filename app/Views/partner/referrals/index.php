<!-- E:\call_center\app\Views\partner\referrals\index.php -->
...
<form method="GET" class="row g-2 mb-3 mt-1">
    ...
    <div class="col-auto ms-auto">
        <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#submitReferralModal">
            <i class="bi bi-plus-lg me-1"></i>Submit Referral
        </button>
    </div>
</form>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr><th>Business</th><th>Deal Amount</th><th>Status</th><th>Commission</th><th>Commission Status</th><th>Date</th></tr>
                </thead>
                <tbody>
                <?php foreach ($data as $deal): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($comm): ?>
                            <span class="badge <?= $comm['is_paid']?'bg-success':'bg-warning text-dark' ?>"><?= $comm['is_paid']?'Paid':'Pending' ?></span>
                            <?php else: ?>
                ...
                <?php endforeach ?>
                <?php if(empty($data)): ?>
                    <tr><td colspan="6" class="text-center text-muted py-4">No referrals submitted yet.</td></tr>
                <?php endif ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="modal-dialog">
        <form method="POST" action="<?= APP_URL ?>/partner/referrals/submit">
            ...
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Submit Referral</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-12"><label class="form-label">Company Name *</label><input type="text" name="company_name" class="form-control" required></div>
                        ...
                        <div class="col-md-6"><label class="form-label">Est. Value (€) *</label><input type="number" name="amount" class="form-control" step="0.01" required min="1"></div>
                        <div class="col-12"><label class="form-label">Notes</label><textarea name="notes" class="form-control" rows="2"></textarea></div>
                    </div>
                </div>
                <div class="modal-footer"><button type="submit" class="btn btn-primary"><i class="bi bi-send me-1"></i>Submit</button></div>
            </div>
        </form>
    </div>
</div>
