...
<div class="d-flex justify-content-between align-items-center mt-2 mb-3">
    <a href="<?= APP_URL ?>/admin/projects" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Όλα τα Έργα</a>
    <div class="d-flex gap-2">
        <a href="<?= APP_URL ?>/admin/projects/<?= $project['id'] ?>/edit" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil me-1"></i>Επεξεργασία</a>
    </div>
</div>
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <span class="fw-semibold"><i class="bi bi-kanban me-1"></i> Στοιχεία Έργου</span>
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        <table class="table table-borderless table-sm mb-0">
                            <tr><th class="text-muted fw-normal w-40">Επιχείρηση</th><td class="fw-semibold"><?= htmlspecialchars($project['company_name']) ?></td></tr>
                            <tr><th class="text-muted fw-normal">Πόλη</th><td><?= htmlspecialchars($project['city'] ?? '—') ?></td></tr>
                            <tr><th class="text-muted fw-normal">Τηλεφωνητής</th><td><?= htmlspecialchars($project['caller_name']) ?></td></tr>
                            <tr><th class="text-muted fw-normal">Προγραμματιστής</th><td><?= htmlspecialchars($project['developer_name'] ?? '—') ?></td></tr>
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless table-sm mb-0">
                            <tr><th class="text-muted fw-normal w-40">Προϋπολογισμός</th><td class="fw-bold text-success fs-5">€<?= number_format($project['budget'],2) ?></td></tr>
                            <tr><th class="text-muted fw-normal">Ημ. Έναρξης</th><td><?= $project['start_date'] ? date('d M Y', strtotime($project['start_date'])) : '—' ?></td></tr>
                            <tr><th class="text-muted fw-normal">Προθεσμία</th><td><?= $project['deadline'] ? date('d M Y', strtotime($project['deadline'])) : '—' ?></td></tr>
                            <tr><th class="text-muted fw-normal">Ολοκλήρωση</th><td><?= $project['actual_end'] ? date('d M Y', strtotime($project['actual_end'])) : '—' ?></td></tr>
                            <tr><th class="text-muted fw-normal">Σύμβαση</th><td><?= $project['contract_signed'] ? '<span class="badge bg-success">Υπογεγραμμένη</span>' : '<span class="badge bg-warning text-dark">Εκκρεμής</span>' ?></td></tr>
                        </table>
                    </div>
                </div>
                ...
                <hr><strong>Τεχνολογίες:</strong> <?= htmlspecialchars($project['tech_stack']) ?>
                <?php endif ?>
                ...
                <div class="d-flex flex-wrap gap-2">
                    <?php if($project['repo_url']): ?><a href="<?= htmlspecialchars($project['repo_url']) ?>" target="_blank" class="btn btn-sm btn-outline-dark"><i class="bi bi-github me-1"></i>Αποθετήριο</a><?php endif ?>
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <span class="fw-semibold"><i class="bi bi-list-check me-1"></i> Φάσεις</span>
                <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addPhaseModal">
                    <i class="bi bi-plus-lg me-1"></i>Προσθήκη Φάσης
                </button>
            </div>
            <div class="card-body p-0">
                ...
                <table class="table table-hover mb-0">
                    <thead class="table-light"><tr><th>#</th><th>Φάση</th><th>Κατάσταση</th><th>Προθεσμία</th><th>Ολοκλήρωση</th><th></th></tr></thead>
                    <tbody>
                    <?php foreach ($phases as $i => $phase): ?>
                        <tr>
                            ...
                            <td>
                                <form method="POST" action="<?= APP_URL ?>/admin/projects/phase/<?= $phase['id'] ?>/delete" class="d-inline" onsubmit="return confirm('Διαγραφή φάσης;')">
                                    <?= CSRF::field() ?>
            ...
            <div class="card-body">
                <form method="POST" action="<?= APP_URL ?>/admin/projects/note/add" class="mb-3">
                    ...
                    <div class="d-flex gap-2 align-items-start">
                        ...
                        <div>
                            <button class="btn btn-primary btn-sm mb-1 w-100">Δημοσίευση</button>
                            <div class="form-check ms-1">
                                <input type="checkbox" name="is_internal" value="1" id="is_internal_cb" class="form-check-input">
                                <label for="is_internal_cb" class="form-check-label small">Εσωτερική</label>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                <div class="border rounded p-2 mb-2 <?= $note['is_internal'] ? 'bg-light border-warning' : '' ?>">
                    <div class="d-flex justify-content-between mb-1">
                        ...
                        <span class="text-muted small"><?= date('d M Y H:i', strtotime($note['created_at'])) ?> <?= $note['is_internal'] ? '<span class="badge bg-warning text-dark ms-1">Εσωτερική</span>' : '' ?></span>
                    </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-semibold"><i class="bi bi-person-badge me-1"></i> Κύριος Προγραμματιστής</div>
            <div class="card-body">
                <form method="POST" action="<?= APP_URL ?>/admin/projects/<?= $project['id'] ?>/assign-developer">
                    ...
                    <button class="btn btn-sm btn-primary w-100"><i class="bi bi-person-check me-1"></i>Ανάθεση</button>
                </form>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-semibold d-flex align-items-center justify-content-between">
                <span><i class="bi bi-people me-1"></i> Ομάδα Έργου</span>
            ...
            <div class="collapse" id="addAssignmentForm">
                <form method="POST" action="<?= APP_URL ?>/admin/projects/<?= $project['id'] ?>/assign-user"
                    ...
                    <?= CSRF::field() ?>
                    <div class="mb-2">
                        <label class="form-label text-xs fw-600">Χρήστης</label>
                        <select name="user_id" class="form-select form-select-sm" required>
                            ...
                            ?>
                            <option value="<?= $u['id'] ?>"><?= htmlspecialchars($u['name']) ?> (<?= grRole($u['role']) ?>)</option>
                            <?php endforeach ?>
                        </select>
                    </div>
                    <div class="mb-2">
                        <label class="form-label text-xs fw-600">Ρόλος στο Έργο</label>
                        <select name="role_type" class="form-select form-select-sm">
                            <option value="developer">Προγραμματιστής</option>
                            <option value="caller">Τηλεφωνητής</option>
                            <option value="partner">Συνεργάτης</option>
                        </select>
                    </div>
                    <div class="mb-2">
                        <label class="form-label text-xs fw-600">Σημειώσεις</label>
                        <input type="text" name="notes" class="form-control form-control-sm" placeholder="Προαιρετικές σημειώσεις…">
                    </div>
                    <button type="submit" class="btn btn-sm btn-primary w-100">
                        <i class="bi bi-person-plus me-1"></i>Προσθήκη στην Ομάδα
                    </button>
                </form>
            </div>
            <div class="card-body p-0">
                <?php if (empty($assignments)): ?>
                <div class="text-center text-muted py-3 small">Δεν έχουν ανατεθεί μέλη ομάδας.</div>
                <?php else: ?>
                    ...
                    <li class="list-group-item px-3 py-2">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                ...
                                <div class="d-flex align-items-center gap-1">
                                    <span class="badge bg-secondary" style="font-size:.68rem">
                                        <i class="bi <?= $roleIcons[$a['role_type']] ?? 'bi-person' ?> me-1"></i><?= grRole($a['role_type']) ?>
                                    </span>
                            ...
                            <form method="POST" action="<?= APP_URL ?>/admin/projects/assignment/<?= $a['id'] ?>/remove"
                                  onsubmit="return confirm('Αφαίρεση από την ομάδα;')" class="flex-shrink-0">
                                <?= CSRF::field() ?>
                                <input type="hidden" name="project_id" value="<?= $project['id'] ?>">
        ...
        <!-- Οικονομική Ανάλυση -->
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-semibold"><i class="bi bi-currency-euro me-1"></i> Οικονομική Σύνοψη</div>
            <div class="card-body">
                <?php
                ...
                ?>
                <table class="table table-borderless table-sm mb-0">
                    <tr><td class="text-muted">Ποσό Συμφωνίας</td><td class="fw-bold text-success text-end">€<?= number_format($dealAmnt,2) ?></td></tr>
                    <tr><td class="text-muted">Προμήθεια Τηλεφωνητή (10%)</td><td class="text-warning text-end">-€<?= number_format($commTotal,2) ?></td></tr>
                    <?php if($project['developer_id']): ?>
                    <tr><td class="text-muted">Προμήθεια Προγρ/στή (20%)</td><td class="text-warning text-end">-€<?= number_format($devComm,2) ?></td></tr>
                    <?php endif ?>
                    <tr><td class="text-muted">Έξοδα</td><td class="text-danger text-end">-€<?= number_format($totalExpAmt,2) ?></td></tr>
                    <tr class="table-light"><td class="fw-semibold">Καθαρό Κέρδος</td><td class="fw-bold <?= $net>=0?'text-success':'text-danger' ?> text-end">€<?= number_format($net,2) ?></td></tr>
                </table>
            </div>
        </div>
        ...
        <div class="card border-0 shadow-sm mb-3">
            ...
            <div class="card-body">
                <form method="POST" action="<?= APP_URL ?>/admin/documents/contracts/<?= $project['deal_id'] ?>/upload" enctype="multipart/form-data" class="mb-3">
                    ...
                    <input type="text" name="notes" class="form-control form-control-sm mb-2" placeholder="Σημειώσεις (προαιρετικά)">
                    <button class="btn btn-sm btn-outline-primary w-100"><i class="bi bi-upload me-1"></i>Ανέβασμα Σύμβασης</button>
                </form>
                <?php if(empty($contracts)): ?>
                <p class="text-muted small mb-0">Δεν έχουν ανέβει συμβάσεις.</p>
                <?php else: ?>
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item px-0 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <div class="text-muted" style="font-size:.7rem"><?= date('d M Y', strtotime($c['uploaded_at'])) ?> από <?= htmlspecialchars($c['uploader_name']) ?></div>
                        </div>
        ...
        <div class="card border-0 shadow-sm mb-3">
            ...
            <div class="card-body">
                ...
                <p class="text-muted small mb-0">Δεν έχουν δημιουργηθεί τιμολόγια.</p>
                <?php else: ?>
                <?php foreach ($invoices as $inv): ?>
                <div class="border rounded p-2 mb-2">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <span class="badge <?= match($inv['status']){'draft'=>'bg-secondary','issued'=>'bg-info text-dark','sent'=>'bg-primary','paid'=>'bg-success',default=>'bg-secondary'} ?>"><?= grStatus($inv['status']) ?></span>
                    ...
                    <div class="mt-1 d-flex gap-1">
                        ...
                        <form method="POST" action="<?= APP_URL ?>/admin/documents/invoices/<?= $inv['id'] ?>/mark-paid">
                            <?= CSRF::field() ?><button class="btn btn-xs btn-outline-success" style="font-size:.7rem;padding:2px 6px">Σήμανση Πληρωμένου</button>
                        </form>
                        <?php endif ?>
                        <?php if($inv['filename']): ?>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white fw-semibold"><i class="bi bi-wallet2 me-1"></i> Έξοδα Έργου</div>
            <div class="card-body p-0">
                <?php if(empty($expenses)): ?>
                <div class="text-center text-muted py-3 small">Δεν έχουν καταγραφεί έξοδα.</div>
                <?php else: ?>
                <table class="table table-sm mb-0">
                    <tbody>
                    <?php foreach ($expenses as $exp): ?>
                        <tr>
                            <td><small><?= htmlspecialchars($exp['description']) ?></small><br><span class="badge bg-secondary"><?= grExpCat($exp['category']) ?></span></td>
                ...
                <?php endif ?>
                <div class="p-2 text-end fw-bold border-top">Σύνολο: €<?= number_format($totalExpAmt,2) ?></div>
            </div>
        </div>
    </div>
</div>

<!-- Modal Προσθήκης Φάσης -->
    ...
    <div class="modal-dialog">
        <form method="POST" action="<?= APP_URL ?>/admin/projects/phase/add">
            ...
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Προσθήκη Φάσης</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <div class="mb-3"><label class="form-label">Όνομα Φάσης *</label><input type="text" name="name" class="form-control" required></div>
                    <div class="mb-3"><label class="form-label">Περιγραφή</label><textarea name="description" class="form-control" rows="2"></textarea></div>
                    <div class="row g-2">
                        <div class="col"><label class="form-label">Σειρά</label><input type="number" name="order_num" class="form-control" value="<?= count($phases) ?>"></div>
                        <div class="col"><label class="form-label">Προθεσμία</label><input type="date" name="due_date" class="form-control"></div>
                    </div>
                </div>
                <div class="modal-footer"><button type="submit" class="btn btn-primary">Προσθήκη Φάσης</button></div>
            </div>
        </form>
    </div>
</div>

<!-- Modal Προσθήκης Τιμολογίου -->
    ...
    <div class="modal-dialog">
        <form method="POST" action="<?= APP_URL ?>/admin/documents/invoices/<?= $project['deal_id'] ?>/upload" enctype="multipart/form-data">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row g-2">
                        <div class="col-6"><label class="form-label">Αρ. Τιμολογίου</label><input type="text" name="invoice_no" class="form-control" placeholder="ΤΙΜ-001"></div>
                        ...
                        <div class="col-6"><label class="form-label">Ημ. Έκδοσης</label><input type="date" name="issued_at" class="form-control"></div>
                        <div class="col-6"><label class="form-label">Ημ. Λήξης</label><input type="date" name="due_at" class="form-control"></div>
                        <div class="col-12"><label class="form-label">Αρχείο PDF</label><input type="file" name="invoice_file" class="form-control" accept=".pdf"></div>
