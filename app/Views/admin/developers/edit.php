...
foreach ($categories as $cat) {
    if ($cat['id'] == ($developer['category_id'] ?? null)) { $currentCat = $cat; break; }
}
$userId = $developer['id'];
?>

<div class="page-header">
    <h1 class="page-title"><i class="bi bi-code-slash me-2"></i><?= htmlspecialchars($developer['name']) ?></h1>
    <a href="<?= APP_URL ?>/admin/developers" class="btn btn-outline-secondary btn-sm">
...
<div class="row g-4">
    ...
    <div class="col-lg-7">
        <div class="card mb-3">
            ...
            <div class="card-body">
                <form method="POST" action="<?= APP_URL ?>/admin/developers/<?= $developer['id'] ?>/update">
                    <?= CSRF::field() ?>
                    <div class="row g-3">
                        <div class="col-sm-6">
                        ...
                        <div class="col-sm-6">
                            ...
                            <input type="email" name="email" class="form-control" value="<?= htmlspecialchars($developer['email']) ?>" required>
                        </div>
                        <div class="col-sm-6">
                            <label class="form-label">Phone</label>
                            <input type="text" name="phone" class="form-control" value="<?= htmlspecialchars($developer['phone'] ?? '') ?>">
                        </div>
                        <div class="col-sm-3">
                            <label class="form-label">Status</label>
                            <select name="is_active" class="form-select">
                                <option value="1" <?= $developer['is_active'] ? 'selected' : '' ?>>Active</option>
                                <option value="0" <?= !$developer['is_active'] ? 'selected' : '' ?>>Inactive</option>
                            </select>
                        </div>
                        <div class="col-sm-3">
                            ...
                            <select name="category_id" class="form-select">
                                ...
                                <option value="<?= $cat['id'] ?>" <?= ($developer['category_id'] ?? null) == $cat['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="d-flex gap-2 mt-3 justify-content-end">
                        <a href="<?= APP_URL ?>/admin/developers" class="btn btn-outline-secondary">Cancel</a>
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="stat-box text-center">
                    <div class="stat-val"><?= number_format($currentCat['developer_rate'], 1) ?>%</div>
                    <div class="stat-lbl">Developer Commission Rate</div>
                </div>
                <?php else: ?>
                <div class="text-center text-muted py-3 text-sm">
                    <i class="bi bi-award fs-2 d-block mb-2 opacity-25"></i>
                    No category assigned.<br>Using global rate (<?= defined('DEVELOPER_COMMISSION_RATE') ? DEVELOPER_COMMISSION_RATE : 20 ?>%).
                </div>
                <?php endif ?>
            </div>
        </div>
